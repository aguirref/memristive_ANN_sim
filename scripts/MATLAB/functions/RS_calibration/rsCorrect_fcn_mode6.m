function corrected_WM = rsCorrect_fcn_mode6(CPA_settings, sim_settings, dir_n_files, G, mode,digits_order,I_min, I_max,model_ver,memdiode_model,V_read,Rs,partitions)
    ...
    while strcmpi(calibrate_loop,'yes')
        ...
        for cal_criterion_i=1:length(cal_criterion_vec)
            ...
            if ~any(ismember(calc_results_mat(:,1),cal_criterion))
                ...
                save(fullfile(directory2,'compensation_variables.mat'),'G_trasp','RHRS','RLRS','Rs','accuracy_calc','calibrate','cal_criterion','V_read','cant_muestras','norm','partitions','images_t10k','labels_t10k','output_side','directory');
