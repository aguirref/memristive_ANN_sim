function [data_sim] = fit_mean_curve(results_folder, netlist_folder, version, model_name, Vt, It, varargin)
    scale=1;
    kR='yes';
    imaxOpt=[1,1e-1,10];
...
%     
    ...
    QMM_params=table(str2double(imax_opt),str2double(imin_opt),str2double(alphamax_opt), str2double(alphamin_opt), str2double(etaset_opt), str2double(etares_opt), str2double(vset_opt), str2double(vres_opt), str2double(rs_opt));
    QMM_params.Properties.VariableNames = {'imax' 'imin' 'alphamax' 'alphamin' 'etaset' 'etares' 'vset' 'vres' 'rs'};     
    data_sim.QMM_params=QMM_params;
